...
// Хранение информации о пользователях, которые уже запускали /start
const userStates = new Set();
// Обрабатываем команду /start
bot.onText(/\/start/, (msg) => {
     const chatId = msg.chat.id;

     // Проверяем, посещал ли пользователь бот раньше
     if (!userStates.has(chatId)) {
          // Если это первый визит, отправляем приветствие
          const startMessage = `
...
🎧 Приятных покупок!
        `;
          ...
          const imageUrl = 'dokonlogo';  // URL приветственной картинки
          ...
          // Добавляем пользователя в список тех, кто уже запускал команду /start
          userStates.add(chatId);
     } else {
          // Если пользователь уже был, отправляем только короткое сообщение или ничего
          bot.sendMessage(chatId, "Вы уже запускали команду /start.");
     }
});
